...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!url) {
      return invalidRequest("Missing required 'url' parameter");
    }
    ...
    if (isDemoMode) {
      ...
      if (!dryRun) {
        ...
        result.exportPath = `./agents/${result.solution.uniqueName}_demo.zip`;
    ...
    if (!dryRun) {
      ...
      const outputPath = `./agents/${resolved.solution.uniquename}_${timestamp}_${suffix}.zip`;
...
export async function GET(request: NextRequest) {
  try {
    ...
    return invalidRequest(error instanceof Error ? error.message : "Invalid URL");
  }
}

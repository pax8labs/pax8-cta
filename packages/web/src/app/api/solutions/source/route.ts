  ...
  'https://pax8demo.crm.dynamics.com': [
    {
      ...
      description: 'An AI-powered HR assistant for employee questions about policies, benefits, and procedures.',
      isManaged: false,
  ...
  try {
    ...
    if (!environmentUrl) {
      return NextResponse.json({
        demoMode: false,
        configured: false,
        message: 'No environment specified. Select an environment or configure a default source environment in Settings.',
        solutions: [],
      })
    }
    ...
    if (!sourceTenantId) {
      return NextResponse.json(
        { error: 'Unable to determine tenant ID for environment' },
        { status: 500 }
      )
    }
    ...
    let exportableSolutions = solutions
...
async function detectBotInSolution(
  ...
  solutionId: string
): Promise<{ hasBot: boolean; botId?: string; botName?: string; botType?: string; topicsCount?: number; knowledgeSources?: string[] }> {
  try {
    ...
    const result = await client.get<{ value: SolutionComponent[] }>(
      '/solutioncomponents',
      {
    ...
    if (result.value && result.value.length > 0) {
      ...
      if (botComponentId) {
        try {
          ...
          const botResult = await client.get<{ value: ChatBot[] }>(
            '/bots',
            {
    ...
    try {
      const pvaResult = await client.get<{ value: SolutionComponent[] }>(
        '/solutioncomponents',
        {
          $select: 'objectid,componenttype',

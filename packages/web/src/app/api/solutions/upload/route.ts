...
export async function POST(request: NextRequest) {
  ...
  // Apply rate limiting - file uploads can be expensive
  ...
  try {
    ...
    if (!file || !(file instanceof File)) {
      return invalidRequest("Missing required 'file' in form data");
    }

    // Validate file type
    if (!file.name.endsWith(".zip")) {
      return invalidRequest("File must be a .zip file");
    }

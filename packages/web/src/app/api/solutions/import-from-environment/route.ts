  ...
  try {
    ...
    if (!solutionUniqueName || !environmentUrl) {
      return NextResponse.json(
        { error: 'solutionUniqueName and environmentUrl are required' },
        { status: 400 }
      )
    }
    ...
    if (!solution) {
      return NextResponse.json(
        { error: `Solution '${solutionUniqueName}' not found in environment` },
        { status: 404 }
      )
    }
    ...
    return NextResponse.json(
      {
        error: 'Failed to import solution from environment',

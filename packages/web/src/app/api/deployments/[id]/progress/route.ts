...
const logger = createLogger("deployment-progress");
        ...
        if (isDemoMode()) {
          ...
          const tenants = pendingTenants.map((t) => {
            ...
            return {
              ...
              urlOverride: (
                t as {
                  urlOverride?: { sharepoint: string; dynamicsCrm: string; onmicrosoft: string };
                }
              ).urlOverride,
            };
          });
          ...
          if (finalDeployment) {
            ...
            if (allDone) {
              ...
              finalDeployment.completedTenants = finalDeployment.tenantResults.filter(
                (t) => t.status === "completed"
              ).length;
              finalDeployment.failedTenants = finalDeployment.tenantResults.filter(
                (t) => t.status === "failed"
              ).length;
...
async function processTenant(
  ...
  for (const stepId of stepOrder) {
    ...
    const currentTenant = currentDeployment?.tenantResults.find(
  ...
  if (deploymentFinal && deploymentFinal.status !== "cancelled") {
    ...
    deploymentFinal.completedTenants = deploymentFinal.tenantResults.filter(
      (t) => t.status === "completed"
    ).length;
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      queueEvents.on("completed", async ({ jobId, returnvalue }) => {
        ...
        let result:
          | { success: boolean; tenantName?: string; error?: string; durationMs?: number }
          | undefined;
        try {
    ...
    if (isRedisConnectionError(error)) {
      logger.error("Redis connection failed for deployment progress SSE", { error, deploymentId });
      send({
        type: "error",
        message: "Deployment queue unavailable. Please try again in a few moments.",
        deploymentId,
        timestamp: new Date().toISOString(),
      });
    } else {
      logger.error("Deployment progress SSE error", error as Error);
      send({
        type: "error",

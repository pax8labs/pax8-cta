...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (session instanceof NextResponse) {
    logAuthFailure(undefined, `/api/deployments/${id}/rollback`, "forbidden", {
  ...
  try {
    ...
    if (!clientSecret) {
      return internalError(
        "AZURE_CLIENT_SECRET environment variable is required for production rollback"
      );
    }
    ...
    for (const deployment of completedDeployments) {
      try {
        ...
        if (!snapshot) {
          ...
          deploymentRepo.updateDeploymentStatus(
            deployment.id,
            "failed",
        ...
        if (!deployment.environmentUrl) {
          ...
          deploymentRepo.updateDeploymentStatus(
            deployment.id,
            "failed",
            "Deployment missing environment URL"
          );
          continue;
        }
        ...
        if (!tenantConfig) {
          ...
          deploymentRepo.updateDeploymentStatus(
            deployment.id,
            "failed",
            "Tenant not found in configuration"
          );
          continue;
        }

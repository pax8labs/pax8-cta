...
import { loadConfig, TenantConfig, isDemoMode, DEMO_TENANTS, Deployment, DeploymentBatch, DeploymentStatus, getDeploymentNotifications } from '@agentsync/core'
  ...
  try {
    ...
    if (dryRun) {
      ...
      return NextResponse.json({
        ...
        warnings: [
          ...(missingTenants.length > 0 ? [`${missingTenants.length} tenant(s) not found or disabled`] : []),
          ...(!redisHealthy ? ['Redis connection failed - deployments will not be queued'] : []),
        ],
        message: missingTenants.length === 0 && redisHealthy

...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      ...
      return unauthorized(
        'Missing or invalid authorization header. Use "Bearer <webhook-secret>" format.'
      );
    }
    ...
    try {
      ...
      logger.error("Invalid webhook payload", error as Error, {
        invocationId,
        webhookId: webhook.id,
      });
      ...
      return validationError("Invalid webhook payload", {
        details:
          error instanceof Error ? error.message : "Invalid JSON or schema validation failed",
      });
    }

...
function generateIssueTitle(report: ErrorReport): string {
  ...
  const shortMessage =
    normalizedMessage.length > 60 ? normalizedMessage.slice(0, 60) + "..." : normalizedMessage;
...
async function findExistingIssue(title: string): Promise<string | null> {
  ...
  try {
    ...
    const searchQuery = encodeURIComponent(
      `repo:${owner}/${repo} is:issue is:open label:auto-reported "${errorPattern}"`
    );
    ...
    if (!response.ok) {
      console.error(
        "[GitHub Issue Reporter] Failed to search for existing issues:",
        response.status
      );
      return null;
    }
    ...
    console.error(
      "[GitHub Issue Reporter] Error checking for existing issues:",
...
export async function reportErrorToGitHub(report: ErrorReport): Promise<GitHubIssueResponse> {
  ...
  try {
    ...
    console.error(
      "[GitHub Issue Reporter] Failed to create issue:",
      sanitizeErrorForLogging(error)
    );
    return {
      success: false,

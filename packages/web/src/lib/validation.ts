...
export const createDeploymentSchema = z.object({
  solutionPath: z
    ...
    .refine((path) => path.endsWith(".zip"), "Solution must be a .zip file"),
  tenantIds: z
...
export const appSettingsSchema = z.object({
  ...
  emailRecipients: z
    ...
    .or(
      z
        .string()
        .regex(/^[^\s@]+@[^\s@]+\.[^\s@]+(,\s*[^\s@]+@[^\s@]+\.[^\s@]+)*$/, "Invalid email list")
    )
    .optional(),
...
export const updateSettingsSchema = z
  ...
  .refine((data) => data.integration || data.app, {
...
export const updateWebhookSchema = z
  ...
  .refine((data) => data.name || data.enabled !== undefined || data.regenerateSecret, {

...
describe('GitHub Issue Reporter', () => {
  describe('Error hash generation', () => {
    ...
    function generateErrorHash(report: {
      error: Error | string
      errorStack?: string
    }): string {
      const errorMessage =
      ...
      const errorName =
        typeof report.error === 'string' ? 'Error' : report.error.name
  ...
  describe('Issue body formatting', () => {
    function formatIssueBody(report: {
      ...
      sections.push('---')
      sections.push(
        '*This issue was automatically created by the AgentSync error reporter.*'
      )

...
export class AnthropicClient {
  ...
  private async chatWithGemini(
    ...
    let chatHistory = messages.slice(1, -1).filter(m =>
      (m.role !== 'user' || (m.content !== userMessage && !m.content.toUpperCase().includes('CRITICAL INSTRUCTIONS') && !m.content.includes('Example CORRECT response')))
    ).map(msg => ({
    ...
    const isQuotaError = lastError?.status === 429 || errorMessage.includes('quota') || errorMessage.includes('rate limit')

    if (isQuotaError) {
      console.warn('⚠ Gemini API quota exceeded (20 requests/day on free tier), using demo mode responses')
    } else {
    ...
    if (isQuotaError) {
      const prefix = "ℹ️ **Note:** Gemini API quota exceeded (free tier allows 20 requests/day). Using demo mode responses until quota resets.\n\n"
  ...
  private getMockResponseWithActions(userMessage: string, systemContext: string = ''): string | { content: string; toolCalls?: any[] } {
    // IMPORTANT: Search only in the actual user message, not in system context/examples
    const msg = userMessage.toLowerCase()

    // Disclaimer for mock mode
    const mockDisclaimer = "_⚠️ AI assistant offline (daily quota reached). Using basic pattern matching instead._\n\n"
    ...
    const deployMatch = userMessage.match(/deploy\s+(?:the\s+)?([a-z0-9-\s]+?)(?:\s+to|\s+on)\s+([a-z0-9-\s]+)/i)
    ...
    if (deployMatch && deployMatch[1].trim().length > 1 && deployMatch[2].trim().length > 1) {
      ...
      const matchedTenant = tenantMap[tenantName.toLowerCase()] || { name: tenantName, id: '11111111-1111-1111-1111-111111111111' }
  ...
  private getMockResponse(userMessage: string, systemContext: string = ''): string {
    ...
    const dependencyIssuesCount = this.extractNumber(systemContext, /Dependency Issues: (\d+)/) || 0
    ...
    if (msg.includes('failed') || msg.includes('failures') || msg.includes('issue') ||
        msg.includes('problem') || msg.includes('alert') || msg.includes('wrong') ||
        /what.*\d+/.test(msg) || // "what are the 5 issues"
        /tell me about.*\d+/.test(msg)) { // "tell me about the 5 alerts"
      if (failedCount === 0) {
...
${failedDeploymentIds.slice(0, 3).map((id, i) => `
• **Deployment #${id}**
  Failed recently - click to view details and retry`).join('\n')}
...
${activeDeploymentIds.slice(0, 3).map((id, i) => `
• **Deployment #${id}**
  Currently in progress...`).join('\n')}
...
${activeCount > 3 ? `\n...and ${activeCount - 3} more.\n` : ''}All are progressing normally. You can monitor their progress on the deployments page.`
    ...
    if (msg.includes('agent') && (msg.includes('which') || msg.includes('what') || msg.includes('list') ||
        msg.includes('available') || msg.includes('can') || msg.includes('show'))) {
...
Or use the Deployments page to create a deployment with more options.`
    ...
    if (msg.includes('tenant') && (msg.includes('which') || msg.includes('what') || msg.includes('list') ||
        msg.includes('available') || msg.includes('can') || msg.includes('show') || msg.includes('how many'))) {

...
export const ASSISTANT_TOOLS = [
  {
    ...
    description:
      "Deploy an agent/solution to one or more tenants. CRITICAL: You must look up agent and tenant IDs from the system context lists before calling this tool.",
    input_schema: {
      type: "object",
      properties: {
        agent_name: {
          type: "string",
          description:
            'The agent ID (e.g., "product-demo", "faq-bot"). MUST be the id field from Available Agents list, NOT the display name. Look it up first!',
        },
        tenant_identifiers: {
          type: "array",
          items: { type: "string" },
          description:
            "Array of tenant IDs (UUIDs). MUST be the id field from Available Tenants list, NOT the display name. Look them up first!",
        },
        strategy: {
          type: "string",
    ...
    case "get_tenant_health":
      ...
      if (toolInput.tenant_identifiers) {
        return tenants.filter((t: any) =>
          toolInput.tenant_identifiers.some(
            (id: string) => t.id === id || t.name.toLowerCase().includes(id.toLowerCase())
          )
        );
      }
      return tenants;

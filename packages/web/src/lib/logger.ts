...
function redactSensitiveString(str: string): string {
  ...
  redacted = redacted.replace(/AZURE_AD_CLIENT_SECRET\s*[=:]\s*["']?[^"'\s]+["']?/gi, 'AZURE_AD_CLIENT_SECRET=***')

  // Redact GITHUB tokens
  redacted = redacted.replace(/GITHUB_TOKEN\s*[=:]\s*["']?[^"'\s]+["']?/gi, 'GITHUB_TOKEN=***')

  // Redact connection strings
  redacted = redacted.replace(/(Server|Data Source|Password|Pwd|User Id|UID)\s*=\s*[^;]+/gi, '$1=***')
  ...
  const sensitiveKeyPattern = /(^|[\s,;(])([\w]*?(?:password|secret|token|apikey|api[_-]?key|credential)[\w]*?)\s*[=:]\s*["']?([^\s"',;)]+)["']?/gi
...
class Logger {
  ...
  private output(level: LogLevel, entry: LogEntry): void {
    ...
    if (this.isDevelopment) {
      ...
      console[level](`${emoji} [${entry.level.toUpperCase()}] ${entry.message}${contextStr ? ' ' + contextStr : ''}`)
    } else {

-- Initial schema for AgentSync
-- Creates all core tables for deployment management
...
CREATE INDEX IF NOT EXISTS idx_invocations_status ON webhook_invocations(status);
CREATE INDEX IF NOT EXISTS idx_invocations_created ON webhook_invocations(created_at);
...
DROP TABLE IF EXISTS user_tenant_assignments;
DROP TABLE IF EXISTS webhook_invocations;

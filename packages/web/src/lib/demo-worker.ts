...
export function startDemoDeployment(batchId: string) {
  ...
  console.log(`[Demo Worker] Starting simulation for batch ${batchId} with ${deployments.length} deployments`)
...
function completeDeployment(deployment: Deployment, batch: DeploymentBatch) {
  ...
  console.log(`[Demo Worker] Completed deployment ${deployment.id} for ${deployment.tenantName} (${completedCount}/${batch.totalDeployments})`)
}

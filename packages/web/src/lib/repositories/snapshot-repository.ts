...
export function getSnapshot(id: string): Snapshot | null {
  const db = getDatabase()
  const row = db.prepare('SELECT * FROM rollback_snapshots WHERE id = ?').get(id) as DbSnapshot | undefined
...
export function getSnapshotsByDeployment(deploymentId: string): Snapshot[] {
  ...
  const rows = db.prepare(
    'SELECT * FROM rollback_snapshots WHERE deployment_id = ? ORDER BY created_at DESC'
...
export function getSnapshotsByTenant(tenantId: string): Snapshot[] {
  const db = getDatabase()
  const rows = db.prepare(
    'SELECT * FROM rollback_snapshots WHERE tenant_id = ? ORDER BY created_at DESC'
...
export function deleteExpiredSnapshots(): number {
  ...
  const result = db.prepare(
    'DELETE FROM rollback_snapshots WHERE expires_at IS NOT NULL AND expires_at < ?'
  ).run(now)
 ...
 */
export function deleteOldSnapshots(tenantId: string, solutionName: string, keepCount: number): number {
  const db = getDatabase()

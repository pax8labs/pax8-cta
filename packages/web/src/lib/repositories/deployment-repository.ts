...
export function getBatch(id: string): DeploymentBatch | null {
  const db = getDatabase();
  const row = db.prepare("SELECT * FROM deployment_batches WHERE id = ?").get(id) as
    | DbDeploymentBatch
    | undefined;
...
export function getDeploymentsByBatch(batchId: string): Deployment[] {
  const db = getDatabase();
  const rows = db
    .prepare("SELECT * FROM deployments WHERE batch_id = ? ORDER BY created_at")
    .all(batchId) as DbDeployment[];
...
export function getDeploymentsByTenant(tenantId: string): Deployment[] {
  const db = getDatabase();
  const rows = db
    .prepare("SELECT * FROM deployments WHERE tenant_id = ? ORDER BY created_at DESC")
    .all(tenantId) as DbDeployment[];
 ...
 */
export function updateDeploymentStatus(id: string, status: DeploymentStatus, error?: string): void {
  const db = getDatabase();
  const now = new Date().toISOString();

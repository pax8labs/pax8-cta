          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <button
                ...
                aria-label={
                  isExpanded
                    ? `Collapse ${agent.friendlyName} details`
                    : `Expand ${agent.friendlyName} details`
                }
              >
                <svg
                  className="w-4 h-4"
            ...
            <div className="flex items-center gap-4 text-xs text-slate-400 ml-6">
              ...
              {agent.lastPublished && (
                <span suppressHydrationWarning>
                  published {formatRelativeTime(new Date(agent.lastPublished))}
                </span>
              )}
            </div>
              ...
              {agent.totalDeployments > 0 ? (
                <>
                  ...
                  {stats.active === agent.totalDeployments ? (
                    ...
                    <span
                      className={`tabular-nums ${stats.health && stats.health >= 70 ? "text-amber-600" : "text-rose-600"}`}
                    >
                ...
                <ul className="space-y-1">
                  {agent.connectionReferences.map((ref) => (
                    <li key={ref.name} className="text-xs text-slate-600 flex items-center gap-1">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${ref.required ? "bg-amber-500" : "bg-slate-300"}`}
                      ></span>
                      {ref.name}
              ...
              <div className="space-y-2">
                {showStatusConfirm === "archived" && agent.totalDeployments > 0 && (
                  <div className="p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">
                    <strong>Note:</strong> This agent is deployed to {agent.totalDeployments} tenant
                    {agent.totalDeployments !== 1 ? "s" : ""}. Archiving will automatically
                    uninstall it from all tenants.
                  </div>
                )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      handleChangeStatus(showStatusConfirm);
                    }}
                ...
                {status === "active" && (
                  <>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setShowStatusConfirm("deprecated");
                      }}

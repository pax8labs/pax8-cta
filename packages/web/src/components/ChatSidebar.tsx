...
export function ChatSidebar() {
  ...
  const [confirmationsDisabled, setConfirmationsDisabled] = useLocalStorage(
    SKIP_CONFIRMATIONS_KEY,
    false
  );
  ...
  const handleActionClick = async (action: ChatAction | undefined) => {
    ...
    if (action.type === "deploy") {
      ...
      if (!action.agentName) {
        await sendMessage(
          "❌ Error: Agent name is missing. Please try asking again with the full deployment request."
        );
        return;
      }
      if (!action.tenantIds || action.tenantIds.length === 0) {
        await sendMessage(
          '❌ Error: No tenants specified. Please try asking again like "deploy [agent] to [tenant]".'
        );
        return;
      }
    }
        ...
        <div className="w-[315px] flex flex-col h-full">
          ...
          <div className="border-b border-gray-300 dark:border-gray-800 bg-white dark:bg-[#252526]">
            <div className="flex items-center gap-3 px-4 py-2.5">
              ...
              <h2 className="text-sm font-medium text-gray-700 dark:text-gray-300 tracking-tight flex-shrink-0">
                Assistant
              </h2>
            ...
            <div className="flex items-center gap-2 px-4 pb-2">
              {llmStatus === "offline" ? (
                <span
                  className="flex items-center gap-1.5 text-xs text-amber-700 dark:text-amber-500 bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded"
                  title="AI assistant unavailable - using fallback responses"
                >
                ...
                <span
                  className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-500 bg-gray-50 dark:bg-gray-800/50 px-2 py-0.5 rounded"
          ...
          <div
            ...
            className="flex-1 overflow-y-auto p-3 bg-gray-50 dark:bg-[#1e1e1e]"
          >
          ...
          <ChatInput
            onSend={sendMessage}
            disabled={isLoading}
            placeholder="Ask about deployments, tenants, health..."
          />
        </div>
      </div>
...
function generateContextualPrompt(
  ...
  if (stats?.batchesWithFailures > 0)
    issues.push(`${stats.batchesWithFailures} failed deployments`);
  if (stats?.versionDriftCount > 0)
    issues.push(`${stats.versionDriftCount} tenants with version drift`);
  if (stats?.dependencyIssuesCount > 0)
    issues.push(`${stats.dependencyIssuesCount} tenants with missing dependencies`);
  if (approvals?.deployments?.length > 0)
    issues.push(`${approvals.deployments.length} deployments awaiting approval`);
  ...
  if (pathname?.startsWith("/tenants")) {
    if (stats?.versionDriftCount > 0 || stats?.dependencyIssuesCount > 0) {
      ...
      if (stats?.versionDriftCount > 0)
        tenantIssues.push(`${stats.versionDriftCount} with version drift`);
      if (stats?.dependencyIssuesCount > 0)
        tenantIssues.push(`${stats.dependencyIssuesCount} with missing dependencies`);
  ...
  switch (action.type) {
    case "deploy":
      ...
      if (!solutionResponse.ok) {
        throw new Error(
          `Failed to download solution "${agentId}": ${solutionResponse.status}. Make sure the LLM is using agent IDs, not display names.`
        );
      }

...
describe('useAgentList', () => {
  ...
  describe('filtering', () => {
    it('should filter agents by search query (friendlyName)', () => {
      ...
      expect(result.current.agents.map(a => a.friendlyName)).toEqual(['Sales Bot', 'Support Bot'])
    })
    ...
    it('should filter agents by description', () => {
      const agents = [
        createMockAgent({ id: '1', friendlyName: 'Bot A', description: 'Handles customer inquiries' }),
    ...
    it('should be case-insensitive', () => {
      const agents = [
        createMockAgent({ id: '1', friendlyName: 'Sales Bot' }),
      ]
      mockUseSWR.mockReturnValue({
        data: { agents },
        error: undefined,

...
describe('handleGetDeploymentStatus', () => {
  ...
  it('should throw validation error for invalid deploymentId format', async () => {
    await expect(handleGetDeploymentStatus({ deploymentId: 'invalid@id' })).rejects.toThrow(/Validation failed/);
  });

  it('should handle API errors', async () => {
    vi.mocked(apiClient.get).mockRejectedValue(new Error('Deployment not found'));

    await expect(handleGetDeploymentStatus({ deploymentId: 'batch-123' })).rejects.toThrow('Deployment not found');
  });
});

...
describe('handleCreateDeployment', () => {
  ...
  it('should throw validation error for missing agentId', async () => {
    await expect(
      handleCreateDeployment({ tenantIds: ['tenant-1'] })
    ).rejects.toThrow(/agentId is required/);
  });

  it('should throw validation error for invalid agentId type', async () => {
    await expect(
      handleCreateDeployment({ agentId: 123, tenantIds: ['tenant-1'] })
    ).rejects.toThrow(/agentId is required and must be a string/);
  });

  it('should throw validation error for missing tenantIds', async () => {
    await expect(
      handleCreateDeployment({ agentId: 'product-demo' })
    ).rejects.toThrow(/tenantIds is required/);
  });

...
describe('SecretsManager', () => {
  ...
  describe('AzureKeyVaultProvider (via SecretsManager)', () => {
    ...
    it('should get token from managed identity and fetch secret', async () => {
      ...
      expect(mockFetch).toHaveBeenNthCalledWith(1, expect.stringContaining('metadata/identity/oauth2/token'), expect.objectContaining({
        headers: { 'Metadata': 'true' },
      }));

      // Verify secret request
      expect(mockFetch).toHaveBeenNthCalledWith(2, expect.stringContaining('my-secret'), expect.objectContaining({

...
describe('WaveService', () => {
  ...
  describe('createExecutionPlan', () => {
    ...
    it('should respect wave order', () => {
      const config = createTestConfig({
        tenants: [
          createTestTenant('Tenant A', ['wave2']),
    ...
    it('should only include enabled tenants', () => {
      const config = createTestConfig({
        tenants: [
          createTestTenant('Enabled', [], true),
          createTestTenant('Disabled', [], false),
        ],
      });
  ...
  describe('validateWaveConfig', () => {
    ...
    it('should warn about unknown tenant references', () => {
      const config = createTestConfig({
        ...
        settings: {
          waves: [
            { name: 'First', order: 1, tenants: ['nonexistent'], continueOnFailure: false },
          ],
        },
      });

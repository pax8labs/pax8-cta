...
const URL_PATTERNS: UrlPattern[] = [
  {
    ...
    generateTemplate: (url, _tenant) => url.replace(
  ...
  },
  {
    ...
    generateTemplate: (url, _tenant, region) => url.replace(
...
export class UrlTemplater {
  ...
  resolveTemplate(templatePattern: string, tenantUrls: TenantUrlValues): string {
    ...
    resolved = resolved.replace(
      /\{tenant\}\.sharepoint\.com/g,
      tenantUrls.sharepoint
    )
    resolved = resolved.replace(
      /\{tenant\}\.(crm[0-9]*)\.dynamics\.com/g,
      tenantUrls.dynamicsCrm
    )
    resolved = resolved.replace(
      /\{tenant\}\.onmicrosoft\.com/g,
      tenantUrls.onmicrosoft
    )

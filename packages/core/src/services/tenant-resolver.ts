...
export function resolveTenantUrls(tenant: TenantConfig): TenantUrlValues {
  try {
    ...
    const crmMatch = hostname.match(
      /^([a-zA-Z0-9-]+)\.(crm[0-9]*)\.dynamics\.com$/i
    )
    ...
    const apiMatch = hostname.match(
      /^([a-zA-Z0-9-]+)\.api\.(crm[0-9]*)\.dynamics\.com$/i
    )

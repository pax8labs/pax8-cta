...
class HealthChecker {
   ...
   */
  async checkTenantHealthDetail(context: HealthCheckContext, skipCache = false): Promise<TenantHealthDetail> {
   ...
   */
  async checkMultipleTenants(contexts: HealthCheckContext[], skipCache = false): Promise<TenantHealth[]> {
  ...
  private determineStatus(score: number, issues: HealthIssue[]): 'healthy' | 'warning' | 'critical' {

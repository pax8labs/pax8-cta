...
export async function runInteractiveWizard(configPath: string): Promise<WizardResult> {
  console.log(chalk.cyan.bold("\n🚀 Pax8 CTA Interactive Setup Wizard\n"));
  console.log(
    chalk.white(
      "This wizard will help you set up Pax8 CTA by creating an Azure AD app registration"
    )
  );
...
function generateConfig(options: ConfigOptions): string {
  ...
  let config = `# Pax8 CTA Configuration File

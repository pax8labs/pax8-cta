...
export async function runInteractiveWizard(configPath: string): Promise<WizardResult> {
  console.log(chalk.cyan.bold(`\n🚀 ${PRODUCT_NAME} Interactive Setup Wizard\n`));
  console.log(
    chalk.white(
      `This wizard will help you set up ${PRODUCT_NAME} by creating an Azure AD app registration`
    )
  );
  ...
  try {
    ...
    const appName = `${PRODUCT_NAME} CLI`;
    ...
    if (!ctx.clientSecret) {
      ...
      console.log(chalk.gray(`   ${formatCommandExample("auth login")}`));
      console.log(
        chalk.gray(
    ...
    console.log(chalk.gray(`   ${formatCommandExample("tenants list")}\n`));

    if (selectedEnvironments.length > 0 || selectedTenants.length > 0) {
      console.log(chalk.white("You can now deploy agents with:"));
      console.log(chalk.gray(`   ${formatCommandExample("deploy <agent-name>")}\n`));
    }
...
function generateConfig(options: ConfigOptions): string {
  ...
  let config = `# ${PRODUCT_NAME} Configuration File

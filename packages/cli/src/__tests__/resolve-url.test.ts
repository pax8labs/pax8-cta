...
describe('Resolve URL Command', () => {
  ...
  describe('URL parsing and resolution', () => {
    it('should parse agent URL and show info', async () => {
      ...
      await program.parseAsync([
        'node', 'test', 'resolve-url',
        '--url', 'https://m365.cloud.microsoft/chat/?titleId=test-title-id'
      ]);
  ...
  describe('error handling', () => {
    ...
    it('should handle resolution errors', async () => {
      const mockParseAgentUrl = vi.fn().mockReturnValue({ titleId: 'test' });
      const mockResolveUrlToSolution = vi.fn().mockRejectedValue(
        new Error('Agent not found in environment')
      );

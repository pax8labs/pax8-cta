...
describe('Deployments Command', () => {
  ...
  describe('deployments list', () => {
    it('should have list subcommand', async () => {
      const { deploymentsCommand } = await import('../commands/deployments.js');

      const listCommand = deploymentsCommand.commands.find(
        cmd => cmd.name() === 'list'
      );
  ...
  describe('deployments show', () => {
    it('should have show subcommand', async () => {
      ...
      const showCommand = deploymentsCommand.commands.find(
        cmd => cmd.name() === 'show'
      );
    ...
    it('should display deployment details', async () => {
      ...
      await program.parseAsync([
        'node', 'test', 'deployments', 'show', 'demo-hist-001'
      ]);
    ...
    it('should handle non-existent deployment', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'deployments', 'show', 'non-existent-id'
        ]);
      } catch (error: any) {
  ...
  describe('command options', () => {
    it('should have --status option', async () => {
      ...
      const statusOption = listCommand?.options.find(
        opt => opt.long === '--status'
      );
    ...
    it('should have --tenant option', async () => {
      ...
      const tenantOption = listCommand?.options.find(
        opt => opt.long === '--tenant'
      );
    ...
    it('should have --agent option', async () => {
      ...
      const agentOption = listCommand?.options.find(
        opt => opt.long === '--agent'
      );
    ...
    it('should have --limit option with default value', async () => {
      ...
      const limitOption = listCommand?.options.find(
        opt => opt.long === '--limit'
      );
    ...
    it('should have --offset option with default value', async () => {
      ...
      const offsetOption = listCommand?.options.find(
        opt => opt.long === '--offset'
      );
    ...
    it('should have --since option', async () => {
      ...
      const sinceOption = listCommand?.options.find(
        opt => opt.long === '--since'
      );
...
describe('Deployments Lifecycle Commands', () => {
  ...
  describe('deployments approve', () => {
    it('should have approve subcommand', async () => {
      const { deploymentsCommand } = await import('../commands/deployments.js');
      const approveCommand = deploymentsCommand.commands.find(
        cmd => cmd.name() === 'approve'
      );
    ...
    it('should show not-implemented warning in demo mode', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'deployments', 'approve', 'demo-hist-001'
        ]);
      } catch (error: any) {
  ...
  describe('deployments reject', () => {
    it('should have reject subcommand', async () => {
      const { deploymentsCommand } = await import('../commands/deployments.js');
      const rejectCommand = deploymentsCommand.commands.find(
        cmd => cmd.name() === 'reject'
      );
    ...
    it('should show not-implemented warning in demo mode', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'deployments', 'reject', 'demo-hist-001'
        ]);
      } catch (error: any) {
        expect(error.message).toContain('process.exit(2)');
      }
  ...
  describe('deployments cancel', () => {
    it('should have cancel subcommand', async () => {
      const { deploymentsCommand } = await import('../commands/deployments.js');
      const cancelCommand = deploymentsCommand.commands.find(
        cmd => cmd.name() === 'cancel'
      );
    ...
    it('should cancel deployment in demo mode', async () => {
      ...
      await program.parseAsync([
        'node', 'test', 'deployments', 'cancel', 'demo-hist-001'
      ]);
  ...
  describe('deployments retry', () => {
    it('should have retry subcommand', async () => {
      const { deploymentsCommand } = await import('../commands/deployments.js');
      const retryCommand = deploymentsCommand.commands.find(
        cmd => cmd.name() === 'retry'
      );
    ...
    it('should retry deployment in demo mode', async () => {
      ...
      await program.parseAsync([
        'node', 'test', 'deployments', 'retry', 'demo-hist-001'
      ]);
  ...
  describe('deployments rollback', () => {
    it('should have rollback subcommand', async () => {
      const { deploymentsCommand } = await import('../commands/deployments.js');
      const rollbackCommand = deploymentsCommand.commands.find(
        cmd => cmd.name() === 'rollback'
      );
    ...
    it('should show not-implemented warning in demo mode', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'deployments', 'rollback', 'demo-hist-001'
        ]);
      } catch (error: any) {
        expect(error.message).toContain('process.exit(2)');
      }
  ...
  describe('deployments watch', () => {
    it('should have watch subcommand', async () => {
      const { deploymentsCommand } = await import('../commands/deployments.js');
      const watchCommand = deploymentsCommand.commands.find(
        cmd => cmd.name() === 'watch'
      );
    ...
    it('should watch deployment in demo mode', async () => {
      ...
      await program.parseAsync([
        'node', 'test', 'deployments', 'watch', 'demo-hist-001'
      ]);
    ...
    it('should handle non-existent deployment in watch', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'deployments', 'watch', 'non-existent-id'
        ]);
      } catch (error: any) {
        expect(error.message).toContain('process.exit(1)');
      }
...
describe('Deployments Integration Tests', () => {
  ...
  it('should show deployment details via CLI', async () => {
    const result = await runCliExpectSuccess([
      'deployments', 'show', 'demo-hist-000'
    ]);
  ...
  it('should show approve not-implemented via CLI', async () => {
    const result = await runCliExpectFailure([
      'deployments', 'approve', 'demo-hist-001'
    ]);
  ...
  it('should show reject not-implemented via CLI', async () => {
    const result = await runCliExpectFailure([
      'deployments', 'reject', 'demo-hist-001', '--reason', 'Test rejection'
    ]);
    expect(containsText(result.output, 'not yet implemented')).toBe(true);
    expect(result.exitCode).toBe(2);
  });
  ...
  it('should cancel deployment via CLI', async () => {
    const result = await runCliExpectSuccess([
      'deployments', 'cancel', 'demo-hist-001'
    ]);
  ...
  it('should retry deployment via CLI', async () => {
    const result = await runCliExpectSuccess([
      'deployments', 'retry', 'demo-hist-001'
    ]);
  ...
  it('should show rollback not-implemented via CLI', async () => {
    const result = await runCliExpectFailure([
      'deployments', 'rollback', 'demo-hist-001'
    ]);
    expect(containsText(result.output, 'not yet implemented')).toBe(true);
    expect(result.exitCode).toBe(2);
  });
  ...
  it('should watch deployment via CLI', async () => {
    const result = await runCliExpectSuccess([
      'deployments', 'watch', 'demo-hist-001'
    ]);
    expect(containsText(result.output, 'DEMO MODE')).toBe(true);
  ...
  it('should show rollback not-implemented for any ID via CLI', async () => {
    const result = await runCliExpectFailure([
      'deployments', 'rollback', 'nonexistent-id'
    ]);
    expect(containsText(result.output, 'not yet implemented')).toBe(true);
    expect(result.exitCode).toBe(2);
  });
  ...
  it('should handle watch not found via CLI', async () => {
    const result = await runCliExpectFailure([
      'deployments', 'watch', 'nonexistent-id'
    ]);

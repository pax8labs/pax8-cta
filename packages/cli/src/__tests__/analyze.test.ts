...
describe('Analyze Command', () => {
  ...
  describe('demo mode - analyze all', () => {
    ...
    it('should analyze all enabled destinations', async () => {
      ...
      expect(containsText(cleanOutput, `Analyzing Risk for ${enabledCount} Destinations`)).toBe(true);
  ...
  describe('demo mode - analyze by tags', () => {
    it('should filter destinations by tag', async () => {
      ...
      const enterpriseTenants = DEMO_TENANTS.filter(t =>
        t.enabled && t.tags?.includes('enterprise')
      );

      expect(containsText(cleanOutput, `Analyzing Risk for ${enterpriseTenants.length} Destinations`)).toBe(true);
    });
  ...
  describe('risk levels', () => {
    ...
    it('should show ready to deploy when no issues', async () => {
      ...
      const hasVerdict = containsText(cleanOutput, 'READY TO DEPLOY') ||
  ...
  describe('option aliases', () => {
    it('should accept --agentPackage as alias for --solution', async () => {
      ...
      const agentPackageOption = analyzeCommand.options.find(opt => opt.long === '--agentPackage');
  ...
  describe('risk assessment details', () => {
    ...
    it('should show next steps hint', async () => {
      ...
      // Should show either "agentsync ship" or "Fix the blockers"
      const hasNextStep = containsText(output, 'agentsync ship') ||
                          containsText(output, 'Fix the blockers');

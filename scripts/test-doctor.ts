...
async function testDoctor() {
  ...
  for (const deployment of failedDeployments) {
    ...
    console.log(
      `   Error Category: ${analysis.category.toUpperCase()} (${Math.round(analysis.confidence * 100)}% confidence)`
    );
  ...
  console.log(
    `  • ${Math.round((criticalCount / analyses.length) * 100)}% of failures are CRITICAL priority`
  );
  console.log(`  • ${Math.round((autoFixableCount / analyses.length) * 100)}% can be auto-fixed`);
  console.log(
    `  • ${fleetInsights.length} fleet-wide pattern${fleetInsights.length !== 1 ? "s" : ""} detected`
  );
  console.log();
}
